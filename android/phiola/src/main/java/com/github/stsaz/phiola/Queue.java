...
class PhiolaQueue {
    ...
    int count() {
        return phi.quCmd(q, Phiola.QUCOM_COUNT, 0);
    }

    int index_real(int i) {
        return phi.quCmd(q, Phiola.QUCOM_INDEX, i);
    }

    Phiola.Meta meta(int i) {
        return phi.quMeta(q, i);
    }
...
class Queue {
    ...
    String get(int i) {
        return queues.get(selected).url(i);
    }
    ...
    int count() {
        return queues.get(selected).count();
    }

    int visiblelist_itemcount() {
        return q_visible().count();
    }
    ...
    boolean auto_stop_armed() {
        return (auto_stop_timer != null);
    }
    ...
    long q_active_id() {
        return queues.get(q_active).q;
    }

    /** Currently selected (not filtered) list */
    private PhiolaQueue q_selected() {
